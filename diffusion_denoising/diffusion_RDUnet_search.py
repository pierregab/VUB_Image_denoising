...
def initialize_model_and_optimizer(base_filters, timesteps):
    ...
    return model_checkpointed, optimizer, scheduler
...
def train_model_checkpointed(model, train_loader, val_loader, optimizer, scheduler, writer, num_epochs=10, 
                             start_epoch=0, accumulation_steps=4, clip_value=1.0):
...
def objective(trial):
    ...
    base_filters = trial.suggest_int('base_filters', 4, 64, step=4)
    timesteps = trial.suggest_int('timesteps', 5, 50, step=5)

    # Initialize model, optimizer, and scheduler with suggested hyperparameters
    model, optimizer, scheduler = initialize_model_and_optimizer(base_filters, timesteps)

    log_dir = os.path.join("runs", "diffusion_optuna_trial_{}".format(trial.number))
    ...
    # Train model
    train_model_checkpointed(model, train_loader, val_loader, optimizer, scheduler, writer, num_epochs=num_epochs, accumulation_steps=accumulation_steps, clip_value=clip_value)

    writer.close()

    # Evaluate the final validation loss
    val_loss = combined_loss(val_loader.dataset[0][1].unsqueeze(0).to(device), model.improved_sampling(val_loader.dataset[0][0].unsqueeze(0).to(device)))
