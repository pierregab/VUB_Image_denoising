...
import pretrained_diffusion.utils.utils_sampling as utils_sampling
...
class DiffusionModel(nn.Module):
    ...
    def generalized_steps(self, x, seq, eta):
        return utils_sampling.generalized_steps(x=x, seq=seq, model=self.unet, b=self.betas, eta=eta)

    def improved_sampling(self, noisy_image):
        x_t = noisy_image
        seq = list(range(0, self.timesteps, self.timesteps // 10))  # Adjust sampling steps as needed
        x_t = self.generalized_steps(x_t, seq, eta=0.8)
        return x_t
...
def train_model_checkpointed(model, train_loader, val_loader, optimizer, scheduler, writer, num_epochs=10, start_epoch=0, accumulation_steps=4, clip_value=1.0):
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (noisy_images, clean_images) in enumerate(train_loader):
            noisy_images, clean_images = data_transform(noisy_images.to(device)), data_transform(clean_images.to(device))
        ...
        with torch.no_grad():
            ...
            val_noisy_images, val_clean_images = data_transform(val_noisy_images.to(device)), data_transform(val_clean_images.to(device))
            ...
            val_clean_images = data_transform_reverse(val_clean_images.cpu())
            val_noisy_images = data_transform_reverse(val_noisy_images.cpu())
            denoised_images = data_transform_reverse(denoised_images.cpu())
